...
def get_instance_impact(instance_name, usage_hours, usage_location, load_percentage):
    ...
    provider = "azure"
    if (
        "GEN_IMPACT_PROVIDER" in os.environ
        and os.environ["GEN_IMPACT_PROVIDER"] == "aws"
    ):
        provider = "aws"
    if usage_hours > 0:
    ...
    data = {
        ...
        "usage": {
            ...
            "time_workload": [
                {"time_percentage": 100, "load_percentage": f"{load_percentage}"}
            ],
        },
    }
...
def make_instance_impacts():
    ...
    for load in load_averages:
        final_data = pd.concat(
            [final_data, pd.DataFrame({"energy_1h_load{}".format(load): []})], axis=1
        )
    ...
    for instance in base_data[["id"]].iterrows():
        ...
        for location in usage_locations:
            impacts[location] = {}
            for load in load_averages:
                impacts[location][load] = get_instance_impact(
                    instance_name, 1, location, load
                )
                final_data["gwp_use_1h_load{}_{}".format(load, location)].values[
                    instance[0]
                ] = impacts[location][load]["impacts"]["gwp"]["use"]["value"]
        for load in load_averages:
            final_data["energy_1h_load{}".format(load)].values[instance[0]] = impacts[
                usage_locations[0]
            ][load]["verbose"]["avg_power"]["value"]
        final_data["gwp_manufacturing_1h_impact"].values[instance[0]] = impacts[
            usage_locations[0]
        ][load_averages[0]]["impacts"]["gwp"]["embedded"]["value"]
        final_data["gwp_manufacturing_total_host_lifetime_impact"].values[
            instance[0]
        ] = impacts_total_lifetime["impacts"]["gwp"]["embedded"]["value"]
