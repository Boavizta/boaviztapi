...
class RenewableEnergyProvider(ElectricityMapsService):
    ...
    def get_cache_scheduler(temporalGranularity: str = 'hourly') -> CacheService:
        ...
        if temporalGranularity.lower() in ['15_minutes', 'hourly']:
            for country in ElectricityCostsProvider.get_eic_countries():
                url = f"{ElectricityMapsService.base_url}/renewable-energy/latest?zone={country.zone_code}&temporalGranularity={temporalGranularity}"
            ...
            datetime_parameter = (datetime.now() - timedelta(
                ...
                "%Y-%m-%dT%H:%M:00Z")
            for country in ElectricityCostsProvider.get_eic_countries():
                url = f"{ElectricityMapsService.base_url}/renewable-energy/past?zone={country.zone_code}&datetime={datetime_parameter}&temporalGranularity={temporalGranularity}"
                endpoints.append(url)
        api_token = ElectricityMapsService._get_api_key()
        cache_service = CacheService(name=f"electricity_renewable_energy_cache_{temporalGranularity}",
                                     endpoints=endpoints,
