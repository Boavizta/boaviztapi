...
from boaviztapi.models.component.functional_block import get_functional_block
...
def mapper_iot_device(dto_iot: IoT, archetype):
    ...
    for functional_block in dto_iot.functional_blocks:
        ...
        model_functional_block = class_functional_block(
            archetype=get_arch_component(archetype, class_functional_block.NAME)
        )
    ...
    if dto_iot.usage is not None:
        model.usage = mapper_usage(
            dto_iot.usage, archetype=get_arch_component(archetype, "USAGE")
        )
