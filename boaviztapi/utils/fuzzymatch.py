...
def fuzzymatch_attr_from_gpu_name(name: str, df: pd.DataFrame) -> Optional[Tuple[Any, Any, Any, Any, Any, Any]]:
    name_ = name.lower()
    score = df["name"].str.lower().apply(lambda x: fuzz.token_set_ratio(x, name_))
    max_score = score.max()
