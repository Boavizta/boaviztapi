...
from boaviztapi.models import ComputedImpacts
from boaviztapi.models.boattribute import Boattribute
from boaviztapi.models.device.server import DeviceServer
from boaviztapi.models.services.cloud_instance import Service, ServiceCloudInstance
...
from boaviztapi.models.component.functional_block import ComponentFunctionalBlock
from boaviztapi.models.device import Device
from boaviztapi.models.device.iot import DeviceIoT
from boaviztapi.models.impact import (
    ImpactFactor,
    IMPACT_PHASES,
    IMPACT_CRITERIAS,
    Impact,
...
def compute_single_impact(
    ...
    try:
        ...
        impact, min_impact, max_impact, warnings = impact_function(
            criteria, duration, model
        )
...
def not_implemented_function(
    impact_type: str, duration: int, model: Union[Component, Device, Service]
):
    raise NotImplementedError


def simple_impact_use(
    impact_type: str, duration: int, model: Union[Component, Device, Service]
) -> ComputedImpacts:
    ...
    impacts = (
        impact_factor.value
        * (model.usage.avg_power.value / 1000)
        * model.usage.use_time_ratio.value
        * duration
        * model.units.value
    )
    max_impact = (
...
def simple_embedded(
    impact_type: str, duration: int, model: [Device, Component, Service]
) -> ComputedImpacts:
    if hasattr(model, "type") and model.type is not None:
        impact = Impact(
            value=float(
                get_impact_factor(item=model.NAME, impact_type=impact_type)[
                    model.type.value
                ]["impact"]
            )
            * model.units.value,
        ...
        impact = Impact(
            value=float(
                get_impact_factor(item=model.NAME, impact_type=impact_type)["impact"]
            )
            * model.units.value,
            min=float(
                get_impact_factor(item=model.NAME, impact_type=impact_type)["impact"]
...
def cpu_impact_use(
    impact_type: str, duration: int, cpu: ComponentCPU
) -> ComputedImpacts:
    ...
    impact = Impact(
        ...
        * (cpu.usage.avg_power.value / 1000)
        * cpu.usage.use_time_ratio.value
        * duration,
        min=impact_factor.min
        * (cpu.usage.avg_power.min / 1000)
        * cpu.usage.use_time_ratio.min
        * duration,
        max=impact_factor.max
        * (cpu.usage.avg_power.max / 1000)
        * cpu.usage.use_time_ratio.max
        * duration,
    )
...
def assembly_impact_embedded(
    impact_type: str, duration: int, model: ComponentAssembly
) -> ComputedImpacts:
    impact = Impact(
...
def casing_impact_embedded(
    impact_type: str, duration: int, case: ComponentCase
) -> ComputedImpacts:
...
def impact_manufacture_rack(impact_type: str, case: ComponentCase) -> ComputedImpacts:
    ...
    return (
        impact_factor.value * case.units.value,
        impact_factor.min * case.units.min,
        impact_factor.max * case.units.max,
        ["End of life is not included in the calculation"],
    )
...
def impact_manufacture_blade(impact_type: str, case: ComponentCase) -> ComputedImpacts:
    ...
    impact = compute_impact_manufacture_blade(
        impact_blade_server, impact_blade_16_slots
    )
    ...
    return (
        impact.value * case.units.value,
        impact.min * case.units.min,
        impact.max * case.units.max,
        ["End of life is not included in the calculation"],
    )
...
def get_impact_constants_blade(impact_type: str) -> Tuple[Impact, Impact]:
    impact_blade_server = Impact(
        value=get_impact_factor(item="case", impact_type=impact_type)["blade"][
            "impact_blade_server"
        ],
        min=get_impact_factor(item="case", impact_type=impact_type)["blade"][
            "impact_blade_server"
        ],
        max=get_impact_factor(item="case", impact_type=impact_type)["blade"][
            "impact_blade_server"
        ],
    )
    impact_blade_16_slots = Impact(
        value=get_impact_factor(item="case", impact_type=impact_type)["blade"][
            "impact_blade_16_slots"
        ],
        min=get_impact_factor(item="case", impact_type=impact_type)["blade"][
            "impact_blade_16_slots"
        ],
...
def compute_impact_manufacture_blade(
    impact_blade_server: Impact, impact_blade_16_slots: Impact
) -> ImpactFactor:
    return ImpactFactor(
...
def iot_functional_blocks_impact_embedded(
    impact_type: str, duration: int, function_blocks: ComponentFunctionalBlock
) -> ComputedImpacts:
    impact = Impact(
        value=get_iot_impact_factor(
            function_blocks.IMPACT_KEY, function_blocks.hsl_level.value, impact_type
        )
        * function_blocks.units.value,
        min=get_iot_impact_factor(
            function_blocks.IMPACT_KEY, function_blocks.hsl_level.value, impact_type
        )
        * function_blocks.units.min,
        max=get_iot_impact_factor(
            function_blocks.IMPACT_KEY, function_blocks.hsl_level.value, impact_type
        )
        * function_blocks.units.max,
    )
...
def hdd_impact_embedded(
    impact_type: str, duration: int, hdd: ComponentHDD
) -> ComputedImpacts:
    impact = Impact(
...
def power_supply_impact_embedded(
    impact_type: str, duration: int, power_supply: Component
) -> ComputedImpacts:
    if isinstance(power_supply, ComponentFunctionalBlock):
        return iot_functional_blocks_impact_embedded(
            impact_type, duration, power_supply
        )
...
def server_power_supply_impact_embedded(
    impact_type: str, duration: int, power_supply: ComponentPowerSupply
) -> ComputedImpacts:
    impact_factor = Impact(
    ...
    impact = Impact(
        value=power_supply.unit_weight.value
        * impact_factor.value
        * power_supply.units.value,
...
def ram_impact_embedded(
    impact_type: str, duration: int, ram: ComponentRAM
) -> ComputedImpacts:
    ...
    impact = Impact(
        value=(
            (ram.capacity.value / ram.density.value) * ram_die_impact.value
            + ram_impact.value
        )
        * ram.units.value,
...
def ram_impact_use(
    ...
    impact = ImpactFactor(
        value=impact_factor.value
        * (ram.usage.avg_power.value / 1000)
        * ram.usage.use_time_ratio.value
        * duration,
        min=impact_factor.min
        * (ram.usage.avg_power.min / 1000)
        * ram.usage.use_time_ratio.min
        * duration,
        max=impact_factor.max
        * (ram.usage.avg_power.max / 1000)
        * ram.usage.use_time_ratio.max
        * duration,
    )
...
def ssd_impact_embedded(
    impact_type: str, duration: int, ssd: ComponentSSD
) -> ComputedImpacts:
    ...
    impact = Impact(
        value=(
            (ssd.capacity.value / ssd.density.value) * ssd_die_impact.value
            + ssd_impact.value
        )
        * ssd.units.value,
...
def iot_impact_embedded(
    impact_type: str, duration: int, iot_device: DeviceIoT
) -> ComputedImpacts:
    impacts = []
    min_impacts = []
    max_impacts = []
    ...
    for component in iot_device.components:
        single_impact = compute_single_impact(
            component, "embedded", impact_type, duration
        )
    ...
    return (
        sum(impacts) * iot_device.units.value,
        sum(min_impacts) * iot_device.units.min,
        sum(max_impacts) * iot_device.units.max,
        warnings,
    )
...
def iot_impact_use(
    ...
    impact = (
        impact_factor.value
        * (iot_device.usage.avg_power.value / 1000)
        * iot_device.usage.use_time_ratio.value
        * duration
        * iot_device.units.value
    )
    min_impact = (
        impact_factor.min
        * (iot_device.usage.avg_power.min / 1000)
        * iot_device.usage.use_time_ratio.min
        * duration
        * iot_device.units.min
    )
    max_impact = (
        impact_factor.max
        * (iot_device.usage.avg_power.max / 1000)
        * iot_device.usage.use_time_ratio.max
        * duration
        * iot_device.units.max
    )

    return impact, min_impact, max_impact, []


def server_impact_embedded(
    impact_type: str, duration: int, server: DeviceServer
) -> ComputedImpacts:
    impacts = []
    min_impacts = []
    max_impacts = []
    ...
    try:
        ...
        impact = Impact(
            value=get_impact_factor(item="SERVER", impact_type=impact_type)["impact"],
        ...
        return (
            impact.value * server.units.value,
            impact.min * server.units.min,
            impact.max * server.units.max,
            warnings,
        )


def server_impact_use(
    impact_type: str, duration: int, server: DeviceServer
) -> ComputedImpacts:
    ...
    impact = (
        impact_factor.value
        * (server.usage.avg_power.value / 1000)
        * server.usage.use_time_ratio.value
        * duration
        * server.units.value
    )
    min_impact = (
        impact_factor.min
        * (server.usage.avg_power.min / 1000)
        * server.usage.use_time_ratio.min
        * duration
        * server.units.min
    )
    max_impact = (
        impact_factor.max
        * (server.usage.avg_power.max / 1000)
        * server.usage.use_time_ratio.max
        * duration
        * server.units.max
    )
...
def cloud_impact_embedded(
    impact_type: str, duration: int, cloud_instance: ServiceCloudInstance
) -> ComputedImpacts:
    impacts = []
    min_impacts = []
    max_impacts = []
    warnings = []
    default_allocation = (
        cloud_instance.vcpu.value / cloud_instance.platform.get_total_vcpu()
    )

    try:
        for component in cloud_instance.platform.components:
            component.usage.hours_life_time = (
                cloud_instance.platform.usage.hours_life_time
            )
            allocation = default_allocation
            if component.NAME == "RAM":
                allocation = (
                    cloud_instance.memory.value
                    / cloud_instance.platform.get_total_memory()
                )
            if component.NAME == "SSD":
                if cloud_instance.ssd_storage.has_value():
                    allocation = (
                        cloud_instance.ssd_storage.value
                        / cloud_instance.platform.get_total_disk_capacity("SSD")
                    )
                else:
                    continue
            if component.NAME == "HDD":
                if cloud_instance.hdd_storage.has_value():
                    allocation = (
                        cloud_instance.hdd_storage.value
                        / cloud_instance.platform.get_total_disk_capacity("HDD")
                    )
                else:
                    continue
        ...
        return (
            impact.value * default_allocation,
            impact.min * default_allocation,
            impact.max * default_allocation,
            warnings,
        )


def cloud_impact_use(
    impact_type: str, duration: int, cloud_instance: ServiceCloudInstance
) -> ComputedImpacts:
    ...
    impact = (
        impact_factor.value
        * (cloud_instance.usage.avg_power.value / 1000)
        * platform.usage.use_time_ratio.value
        * duration
    )
    min_impact = (
        impact_factor.min
        * (cloud_instance.usage.avg_power.min / 1000)
        * platform.usage.use_time_ratio.min
        * duration
    )
    max_impact = (
        impact_factor.max
        * (cloud_instance.usage.avg_power.max / 1000)
        * platform.usage.use_time_ratio.max
        * duration
    )
...
impacts_functions = {
    ...
    "ASSEMBLY": {"use": not_implemented_function, "embedded": assembly_impact_embedded},
    "CASE": {"use": simple_impact_use, "embedded": casing_impact_embedded},
    "MOTHERBOARD": {"use": simple_impact_use, "embedded": motherboard_impact_embedded},
    "SERVER": {"use": server_impact_use, "embedded": server_impact_embedded},
    "IOT_DEVICE": {"use": iot_impact_use, "embedded": iot_impact_embedded},
    "LAPTOP": {"use": simple_impact_use, "embedded": simple_embedded},
    "DESKTOP": {"use": simple_impact_use, "embedded": simple_embedded},
    "TABLET": {"use": simple_impact_use, "embedded": simple_embedded},
    "SMARTPHONE": {"use": simple_impact_use, "embedded": simple_embedded},
    "TELEVISION": {"use": simple_impact_use, "embedded": simple_embedded},
    "SMARTWATCH": {"use": simple_impact_use, "embedded": ram_impact_embedded},
    "BOX": {"use": simple_impact_use, "embedded": simple_embedded},
    "USB_STICK": {"use": simple_impact_use, "embedded": simple_embedded},
    "EXTERNAL_SSD": {"use": simple_impact_use, "embedded": simple_embedded},
    "EXTERNAL_HDD": {"use": simple_impact_use, "embedded": simple_embedded},
    "MONITOR": {"use": simple_impact_use, "embedded": simple_embedded},
    "CLOUD_INSTANCE": {"use": cloud_impact_use, "embedded": cloud_impact_embedded},
    "ACTUATORS": {
