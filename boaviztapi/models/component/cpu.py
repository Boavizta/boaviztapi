...
from boaviztapi.models.impact import ImpactFactor
from boaviztapi.data.archetype import get_component_archetype, get_arch_value
...
class ComponentCPU(Component):
    ...
    def model_power_consumption(self) -> ImpactFactor:
        ...
        self.usage.consumption_profile.compute_consumption_profile_model(
            cpu_manufacturer=self.manufacturer.value,
    ...
    def _complete_die_size(self):
        ...
        # If we have a die_size_per_core and core_units, we can compute the die_size
            ...
            self.die_size.set_completed(
                value=float(self.die_size_per_core.value * self.core_units.value),
        ...
        if self.name.has_value() and not self.name_completion:
            ...
            if compute_min_max:
                ...
                (
                    ...
                    cpu_attributes_min
                    if (cpu_attributes_min is not None)
                    else (None, None, None, None, None, None, None, None, None, None)
                )
                (
                    name_max,
                    ...
                    cpu_attributes_max
                    if (cpu_attributes_max is not None)
                    else (None, None, None, None, None, None, None, None, None, None)
                )
            else:
                (
            ...
            if name is not None:
                self.name.set_completed(
                    name, min=name_min, max=name_max, source="fuzzy match"
                )
            if manufacturer is not None:
                self.manufacturer.set_completed(
                    manufacturer,
                    min=manufacturer_min,
                    max=manufacturer_max,
                    source=f"Completed from name name based on {source}.",
                )
            if family is not None:
                self.family.set_completed(
                    family,
                    min=family_min,
                    max=family_max,
                    source=f"Completed from name name based on {source}.",
            ...
            if model_range is not None:
                self.model_range.set_completed(
                    model_range,
                    min=model_range_min,
                    max=model_range_max,
                    source=f"Completed from name name based on {source}.",
    ...
    def _complete_die_size_from_cpu_specs(self):
        ...
        family = (
            fuzzymatch_attr_from_pdf(self.family.value, "code_name", _cpu_specs)
        ...
        elif len(df.index) == 1 or (
            not df.empty and (df["cores"] == df["cores"].iloc[0]).all()
        ):
            self.die_size.set_completed(
                value=rd.round_to_sigfig(
                    (
                        df["total_die_size"].iloc[0]
                        * self.core_units.value
                        / df["cores"].iloc[0]
                    ),
                    3,
                ),
                min=rd.round_to_sigfig(
                    (
                        df["total_die_size"].min()
                        * self.core_units.min
                        / df["cores"].iloc[0]
                    ),
                    3,
                ),
                max=rd.round_to_sigfig(
                    (
                        df["total_die_size"].max()
                        * self.core_units.max
                        / df["cores"].iloc[0]
                    ),
                    3,
