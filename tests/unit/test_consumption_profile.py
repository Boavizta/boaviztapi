...
MODEL_TEST_DATA_POINTS = [0.0, 25.0, 50.0, 75.0, 100.0]
...
@pytest.mark.parametrize(
    ...
    [("Intel", DEFAULT_CPU_PARAMS), ("AMD", DEFAULT_CPU_PARAMS)],
)
def test_cpu_with_manufacturer_name(
    manufacturer: str, expected_model: Dict[str, float]
):
    cpu_cp = CPUConsumptionProfileModel()
...
def validate_models_approx(
    actual_model: ConsumptionProfileModel,
    expected_model: ConsumptionProfileModel,
    rel: float = 0.2,
):
    actual_results = list(
        map(actual_model.apply_consumption_profile, MODEL_TEST_DATA_POINTS)
    )
    expected_results = list(
        map(expected_model.apply_consumption_profile, MODEL_TEST_DATA_POINTS)
    )
...
def test_cpu_with_model_range(
    model_range: str, expected_model_params: Dict[str, float]
):
    cpu_cp = CPUConsumptionProfileModel()
...
@pytest.mark.parametrize(
    "manufacturer,model_range,expected_model_params",
    [
        (
            "Intel",
            "Xeon Platinum",
...
def test_cpu_with_manufacture_and_model_range(
    manufacturer: str, model_range: str, expected_model_params: Dict[str, float]
):
    cpu_cp = CPUConsumptionProfileModel()
    cpu_cp.compute_consumption_profile_model(cpu_model_range=model_range)
    expected_model = CPUConsumptionProfileModel()
...
def validate_power_approx(
    actual_model: ConsumptionProfileModel,
    expected_workload_power: List[Tuple[float, float]],
):
    for workload, expected_power in expected_workload_power:
        assert actual_model.apply_consumption_profile(workload) == pytest.approx(
            expected_power, rel=0.2
        )


@pytest.mark.parametrize(
    "model_range,workload,expected_workload_power",
    [
        (
            # Xeon Platinum 8124M
            "Xeon Platinum",
...
@pytest.mark.parametrize(
    "model_range,tdp,expected_model_params",
    [
        (
            "Xeon Platinum",
            240,
...
def test_cpu_with_tdp_and_model_range(
    model_range: str, tdp: int, expected_model_params: Dict[str, float]
):
    cpu_cp = CPUConsumptionProfileModel()
