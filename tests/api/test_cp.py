...
async def test_complete_valid_cpu_manufacturer_family_partial():
    await CPUConsumptionProfileTest(
        name="intel xeon", expected=(15.7628, 0.07359, 20.4511, -2.7476)
    ).run()
...
async def test_complete_valid_cpu_manufacturer_family():
    await CPUConsumptionProfileTest(
        name="intel xeon gold 6134", expected=(35.5688, 0.2438, 9.6694, -0.6087)
    ).run()
...
async def test_complete_valid_cpu_overrides_tdp_if_present():
    await CPUConsumptionProfileTest(
        ...
        tdp=100,
        expected=(50.8479, 0.0630, 20.4511, -0.9990),
    ).run()
...
async def test_complete_invalid_cpu_returns_default():
    await CPUConsumptionProfileTest(
        name="foobar", expected=(171.1813, 0.0354, 36.89, -10.13)
    ).run()
