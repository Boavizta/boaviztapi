...
"""
    ...
    with server.run_in_thread():
        assert (
            execute_curl(curl_to_test)
            == '{"a":35.5688,"b":0.2438,"c":9.6694,"d":-0.6087}'
        )  # Change to verify if valid json is returned
...
def test_replace_placeholder_by_json():
    ...
    assert (
        replace_placeholder_by_json(json_exemple, placeholder_to_change) == json_exemple
    )
